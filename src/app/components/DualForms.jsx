...
export default function DualForms() {
  return (
    <div id="dualform" className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
...
function SignupForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/SignUp/v1', {
        method: 'POST',
        headers: {
...
function LeadForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/SignUp/v1/NewProspect', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
